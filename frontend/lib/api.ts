...
/**
 * POST /api/payments/request-otp — unauthenticated.
 * Requests a 6-digit OTP code for a successful registration payment.
 * No Bearer token required; the payment reference alone authorises the request.
 */
export async function apiRequestOtp(
  ...
  const apiUrl = process.env.NEXT_PUBLIC_API_URL;
  if (!apiUrl) {
    throw new ApiRequestError(0, "Payment verification is not available right now.");
  }
  ...
  let res: Response;
  try {
    res = await fetch(`${apiUrl}/api/payments/request-otp`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return ok({ message: json?.message ?? "OTP sent" });
}

/**
 * POST /api/payments/verify-otp — unauthenticated.
 * Verifies the 6-digit OTP and issues a session (access token + refresh cookie)
 * identical to the normal login flow.
 */
export async function apiVerifyOtp(
  ...
  let res: Response;
  try {
    res = await fetch(`${apiUrl}/api/payments/verify-otp`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
